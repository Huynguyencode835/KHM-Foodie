...
if (saveBtn) {
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) {
                (window.showToast || showToast)(result.message || 'Cập nhật thành công!', 'success');
                ...
                (window.showToast || showToast)(result.message || 'Lưu thất bại', 'error');
                saveBtn.innerHTML = originalContent;
                saveBtn.disabled = false;
            }
        } catch (err) {
            (window.showToast || showToast)('Lỗi kết nối đến máy chủ', 'error');
            saveBtn.innerHTML = originalContent;
            saveBtn.disabled = false;
        }
    });
